...
// WebApp
(function () {
  const { Utils } = window.Telegram;
  const { WebView } = window.Telegram;
  const { initParams } = WebView;
  const { isIframe } = WebView;
  ...
  function receiveWebViewEvent(eventType) {
    ...
    WebView.callEventCallbacks(`webview:${eventType}`, (callback) => {
  ...
  function setClosingConfirmation(need_confirmation) {
    if (!versionAtLeast("6.2")) {
      console.warn(
        `[Telegram.WebApp] Closing confirmation is not supported in version ${webAppVersion}`
      );
      return;
    }
  ...
  function setHeaderColor(color) {
    if (!versionAtLeast("6.1")) {
      console.warn(
        `[Telegram.WebApp] Header color is not supported in version ${webAppVersion}`
      );
      return;
    }
    let color_key;
    if (color == "bg_color" || color == "secondary_bg_color") {
      ...
      if (themeParams.bg_color && themeParams.bg_color == color_key) {
        color_key = "bg_color";
      } else if (
    ...
    if (color_key != "bg_color" && color_key != "secondary_bg_color") {
      console.error(
    ...
    WebView.postEvent("web_app_set_header_color", false, { color_key });
  }
  ...
  function setBackgroundColor(color) {
    if (!versionAtLeast("6.1")) {
      console.warn(
        `[Telegram.WebApp] Background color is not supported in version ${webAppVersion}`
      );
      return;
    }
    let bg_color;
  ...
  const BackButton = (function () {
    ...
    function buttonCheckVersion() {
      if (!versionAtLeast("6.1")) {
        console.warn(
          `[Telegram.WebApp] BackButton is not supported in version ${webAppVersion}`
        );
        return false;
      }
      return true;
  ...
  const MainButton = (function () {
    ...
    mainButton.setText = function (text) {
      return mainButton.setParams({ text });
    };
  ...
  const HapticFeedback = (function () {
    ...
    function triggerFeedback(params) {
      if (!versionAtLeast("6.1")) {
        console.warn(
          `[Telegram.WebApp] HapticFeedback is not supported in version ${webAppVersion}`
        );
  ...
  const webAppClipboardRequests = {};
  function onClipboardTextReceived(eventType, eventData) {
    if (eventData.req_id && webAppClipboardRequests[eventData.req_id]) {
      const requestData = webAppClipboardRequests[eventData.req_id];
  ...
  WebApp.switchInlineQuery = function (query, choose_chat_types) {
    if (!versionAtLeast("6.6")) {
      console.error(
        `[Telegram.WebApp] Method switchInlineQuery is not supported in version ${webAppVersion}`
      );
  ...
  WebApp.openInvoice = function (url, callback) {
    ...
    if (!versionAtLeast("6.1")) {
      console.error(
        `[Telegram.WebApp] Method openInvoice is not supported in version ${webAppVersion}`
      );
      throw Error("WebAppMethodUnsupported");
    }
  ...
  WebApp.showPopup = function (params, callback) {
    if (!versionAtLeast("6.2")) {
      console.error(
        `[Telegram.WebApp] Method showPopup is not supported in version ${webAppVersion}`
      );
      throw Error("WebAppMethodUnsupported");
    }
    if (webAppPopupOpened) {
    ...
    if (typeof params.buttons !== "undefined") {
      ...
      for (let i = 0; i < params.buttons.length; i++) {
        ...
        if (
          ...
          if (!text.length) {
            console.error(
              `[Telegram.WebApp] Popup button text is required for type ${button_type}`,
              button.text
            );
            throw Error("WebAppPopupParamInvalid");
          }
  ...
  WebApp.showScanQrPopup = function (params, callback) {
    if (!versionAtLeast("6.4")) {
      console.error(
        `[Telegram.WebApp] Method showScanQrPopup is not supported in version ${webAppVersion}`
      );
      throw Error("WebAppMethodUnsupported");
    }
    if (webAppScanQrPopupOpened) {
  ...
  WebApp.closeScanQrPopup = function () {
    if (!versionAtLeast("6.4")) {
      console.error(
        `[Telegram.WebApp] Method closeScanQrPopup is not supported in version ${webAppVersion}`
      );
      throw Error("WebAppMethodUnsupported");
    }
  ...
  WebApp.readTextFromClipboard = function (callback) {
    if (!versionAtLeast("6.4")) {
      console.error(
        `[Telegram.WebApp] Method readTextFromClipboard is not supported in version ${webAppVersion}`
      );
      throw Error("WebAppMethodUnsupported");
    }
    const req_id = generateId(16);
    const req_params = { req_id };
